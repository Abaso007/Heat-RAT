...
std::string WinApi::read_file(const std::string &file_path)
{
	std::string text;
	std::string line;
...
void WinApi::write_file(const std::string &file_path, const std::string &content)
{
...
bool WinApi::path_exists(const std::string& path)
{
...
std::string WinApi::get_username()
{
	char env[] = "USERNAME";
...
std::string WinApi::last_error_as_string()
{
...
std::vector<std::string> WinApi::listdir(const std::string& path)
{
...
std::string WinApi::random_string()
{
	std::string tmp_s;
...
BITMAPINFOHEADER WinApi::createBitmapHeader(int width, int height)
{
	BITMAPINFOHEADER  bi;
...
HBITMAP WinApi::GdiPlusScreenCapture(HWND hWnd)
{
	...
	BITMAPINFOHEADER bi = WinApi::createBitmapHeader(width, height);
...
bool WinApi::saveToMemory(HBITMAP* hbitmap, std::vector<BYTE>& data, std::string dataFormat)
{
...
std::string WinApi::screenshot(const std::string& path)
{
	std::string full_path = (std::filesystem::path(path) / std::filesystem::path((WinApi::get_date() + ".jpg"))).u8string();
	...
	HBITMAP hBmp = WinApi::GdiPlusScreenCapture(hWnd);

	// save as png to memory
	std::vector<BYTE> data;
	std::string dataFormat = "jpg";

	if (WinApi::saveToMemory(&hBmp, data, dataFormat))
	{
...
void WinApi::append_file(const std::string& file_path, const std::string& content)
{
